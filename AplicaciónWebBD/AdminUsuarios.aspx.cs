...
public partial class AdminUsuarios : System.Web.UI.Page
{
  ...
  public void alta() {
    //Parte 1b: da de alta en la tabla de usuarios.
    GestorBD = (GestorBD.GestorBD)Session["GestorBD"];

    //Verifica que la clave no exista.
    cadSql = "select rfc from PCUsuarios where RFC='" + TxtRFC.Text + "'";
    GestorBD.consBD(cadSql, DsGeneral, "Temp");
    if (DsGeneral.Tables["Temp"].Rows.Count == 0) {
      LblMensaje.Text = "usuario nuevo";
      ...
      if (GestorBD.altaBD(cadSql) == OK) {
        if (DDLTipo.SelectedValue.ToString() == "Cli") {
          //Da de alta en la tabla de clientes.
          ...
          //Da de alta en la tabla de empleados.
      ...
      LblMensaje.Text = "Esta clave ya existe";
